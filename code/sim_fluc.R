...
# Optimum moving following BM, and along SLLR if there is

library(mvtnorm) # To sample from multivariate normal distribution
library(resample) # To use colVars
library(ggplot2)

setwd("/Users/rexjiang/Desktop/novelty/ver_3/serial_homology/out_fluc")
...
# Function to generate a series of optima over time
# Input and output phenotypes are original, not log scale
fluc_bm<-function(n_step,opt_ances,mat_fluc){
  out=matrix(0,nrow=n_step,ncol=length(opt_ances))
  out[1,]=log(opt_ances)
  for(t in 2:n_step){
    out[t,]=out[t-1,]+rmvnorm(1,mean=rep(0,length(opt_ances)),sigma=mat_fluc)[1,]
  }
  return(exp(out))
}
...
sd_fluc_all=c(1,2,4) # Magnitude of optimum movement
...
for(c in 1:nrow(par_all)){
  ...
  for(i in 1:n_rep){
    ...
    z_opt=fluc_bm(n_step,z_a,mat_fluc)
  ...
  z1_var_time[c,]=colVars(log(z1_time));z2_var_time[c,]=colVars(log(z2_time)) # Phenotypic variance
  w_mean_time[c,]=colMeans(w_time) # Mean fitness among populations over time
  w_var_time[c,]=colVars(w_time) # Variance of fitness among populations over time
  
  for(t in 1:n_step){
    cor_time[c,t]=cor(log(z1_time[,t]),log(z2_time[,t])) # Correlation over time
  }
}

write.table(data.frame(par_all,z1_var_time),file="var_time_1_fluc.txt",sep="\t")
write.table(data.frame(par_all,z2_var_time),file="var_time_2_fluc.txt",sep="\t")
write.table(data.frame(par_all,w_mean_time),file="w_mean_time_fluc.txt",sep="\t")
write.table(data.frame(par_all,w_var_time),file="w_var_time_fluc.txt",sep="\t")
write.table(data.frame(par_all,cor_time),file="cor_time_fluc.txt",sep="\t")
write.table(data.frame(par_all,w_geomean),file="w_geomean_fluc.txt",sep="\t")

# Parameter combinations to plot (fluctuation speed, correlational selection)
par_plot_all=rbind(c(1,0),
                   c(1,0.9),
                   c(1,-0.9),
                   c(2,0),
                   c(2,0.9),
...
# Fitness over time
d<-read.table("w_mean_time_fluc.txt",sep="\t")
...
for(row in 1:nrow(par_plot_all)){
  ...
  colnames(d_new)=c("n_p","t","w") # Name the columns
  ...
  # Save the plots
  fn=paste("plot_w_fluc_",par_plot[2],"_",par_plot[1],".pdf",sep="")
  ggsave(fn,plot=g,width=7,height=5)
}

# Fitness variance over time
d<-read.table("w_var_time_fluc.txt",sep="\t")
d$n_p=d$n_p/max(d$n_p) # Convert to fractions
d=d[which(d$r_m==0.9),] # Focus on scenarios where the regulators have similar binding preference
for(row in 1:nrow(par_plot_all)){
  # Extract the row corresponding to the focal parameter combination
  ...
  colnames(d_new)=c("n_p","t","vw") # Name the columns
  d_new=data.frame(d_new) # Convert to data frame
  ...
  g=g+geom_point()+geom_line(lwd=1)+ylim(0,ub)
  g=g+theme_classic()+xlab("")+ylab("") # Clean up axis labels (to be added after combining panels)
  g=g+labs(color=NULL) # Remove legend title (to be manually re-added)
  g=g+guides(color=guide_legend(override.aes=list(shape=15,size=6,linetype=0))) # Replace dots in the legend with boxes
  
  # Save the plots
  fn=paste("plot_vw_fluc_",par_plot[2],"_",par_plot[1],".pdf",sep="")
  ggsave(fn,plot=g,width=7,height=5)
}

# Phenotypic variance over time and phylogenetic signal
d1<-read.table("var_time_1_fluc.txt",sep="\t")
d2<-read.table("var_time_2_fluc.txt",sep="\t")
...
write.table(physig_out,file="physig_fluc.txt",sep="\t")
...
d<-read.table("cor_time_fluc.txt",sep="\t")
d$n_p=d$n_p/max(d$n_p) # Convert to fractions
d=d[which(d$r_m==0.9),] # Focus on scenarios where the regulators have similar binding preference
for(row in 1:nrow(par_plot_all)){
  # Extract the row corresponding to the focal parameter combination
  ...
  g=g+geom_point()+geom_line(lwd=1)+xlim(0,200)+ylim(-1,1)
  g=g+theme_classic()+xlab("")+ylab("") # Clean up axis labels (to be added after combining panels)
  g=g+labs(color=NULL) # Remove legend title (to be manually re-added)
  g=g+guides(color=guide_legend(override.aes=list(shape=15,size=6,linetype=0))) # Replace dots in the legend with boxes
  fn=paste("plot_cor_fluc_",par_plot[2],"_",par_plot[1],".pdf",sep="")
  ggsave(fn,plot=g,width=7,height=5)
}

# Analyze geometric mean fitness
d<-read.table("w_geomean_fluc.txt",sep="\t")
d$n_p=d$n_p/max(d$n_p) # Convert to fractions
d=d[which(d$r_m==0.9),] # Focus on scenarios where the regulators have similar binding preference
...
for(row in 1:nrow(par_plot_all)){
  ...
  colnames(d_new)=c("n_p","geow") # Name the columns
  d_new=data.frame(d_new) # Convert to data frame
  ...
  #g=g+ylim(0,1)+scale_x_continuous(breaks=c(0,0.2,0.4,0.6,0.8,1)) # Specify numbers to show up along x-axis as the independent variable is not factorized
  #g=g+theme_classic()+xlab("")+ylab("") # Clean up axis labels (to be added after combining panels)
  #fn=paste("plot_geo_fluc_",par_plot[2],"_",par_plot[1],".pdf",sep="")
  #ggsave(fn,plot=g,width=7,height=5)
}
signif=cbind(par_plot_all,signif)
colnames(signif)=c("fluc_rate","selection_cor","cor","pv")
write.table(data.frame(signif),file="w_geomean_fluc_cor.txt")

# Extinction risk analysis
d_mean<-read.table("w_mean_time_fluc.txt",sep="\t")#;d_mean$n_p=d_mean$n_p/max(d_mean$n_p);d_mean=d_mean[which(d_mean$r_m==0.9),]
d_var<-read.table("w_var_time_fluc.txt",sep="\t")#;d_var$n_p=d_var$n_p/max(d_var$n_p);d_var=d_var[which(d$r_m==0.9),]
...
# Write data file
write.table(data.frame(d_mean[,1:4],er),file="er_fluc.txt",sep="\t")
write.table(data.frame(d_mean[,1:4],er_cum),file="er_cum_fluc.txt",sep="\t")

d<-read.table("er_fluc.txt",sep="\t") # Re-read data file
d$n_p=d$n_p/max(d$n_p) # Convert to fractions
d=d[which(d$r_m==0.9),] # Focus on scenarios where the regulators have similar binding preference
for(row in 1:nrow(par_plot_all)){
  # Extract the row corresponding to the focal parameter combination
...
d<-read.table("er_cum_fluc.txt",sep="\t") # Re-read data file
d$n_p=d$n_p/max(d$n_p) # Convert to fractions
d=d[which(d$r_m==0.9),] # Focus on scenarios where the regulators have similar binding preference
for(row in 1:nrow(par_plot_all)){
  # Extract the row corresponding to the focal parameter combination
  ...
  # Save the plots
  fn=paste("plot_er_cum_fluc_",par_plot[2],"_",par_plot[1],".pdf",sep="")
  ggsave(fn,plot=g,width=7,height=5)
}
